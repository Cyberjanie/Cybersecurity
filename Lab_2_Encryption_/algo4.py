...
def mix_columns(block):
    ...
    for i in range(0, len(block), 16):
        col = block[i:i+16]
        nibbles = [int(col[j:j+4], 2) for j in range(0, 16, 4)]
        m0 = nibbles[0] ^ nibbles[1]
        m1 = nibbles[1] ^ nibbles[2]
        m2 = nibbles[2] ^ nibbles[3]
        m3 = nibbles[3] ^ nibbles[0]
        mixed += format(m0, '04b') + format(m1, '04b') + format(m2, '04b') + format(m3, '04b')
    return mixed
...
def algo3_cipher():
    print("\n🔐 ALGO4 - Chiffrement à clé 128 bits 🔐")
